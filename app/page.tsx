...
export default async function Home({
  ...
  try {
    ...
    [recent, monthActivities] = await Promise.all([
      getActivities(token, 100),
      getActivitiesSince(token, startOfMonthEpoch),
    ]);
  } catch (e) {
    error = e instanceof Error ? e.message : "Neznámá chyba";
  }

  const athleteName = session.athlete
    ? [session.athlete.firstname, session.athlete.lastname]
        .filter(Boolean)
        .join(" ") || session.athlete.username || "Atlet"
    : "Atlet";
  ...
  return (
    <>
      <YearProgress />
      <aside className="hidden md:flex fixed right-0 top-0 bottom-0 w-16 lg:w-20 flex-col items-center justify-between py-8 z-10 bg-white">
